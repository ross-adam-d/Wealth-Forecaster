...
export default function Projection({ snapshots, scenario, retirementDate, displayReal = true }) {
  ...
  const cashflowData = snapshots.map(s => {
    ...
    return {
      ...
      saleProceedsRouted: transform((s.totalDirectedSaleProceeds ?? 0) - (s.saleProceedsCashContribution ?? 0), s.year),
  ...
  const detailRows = useMemo(() => snapshots.map(s => {
    ...
    return {
      ...
      saleProceedsRouted: (s.totalDirectedSaleProceeds ?? 0) - (s.saleProceedsCashContribution ?? 0),
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      ...
      <div className="card">
        ...
        <ChartFullscreen title={netWorthView === 'networth' ? 'Net Worth' : netWorthView === 'liquidity' ? 'Liquidity' : 'Liquidity Breakdown'}>
          {(isFullscreen) => (
      ...
      <div className="card">
        ...
        <ChartFullscreen title="Annual Cashflow">
          {(isFullscreen) => (
            <div className={isFullscreen ? 'h-full' : 'overflow-x-auto'}>
              <div style={isFullscreen ? { height: '100%' } : { minWidth: '600px' }}>
